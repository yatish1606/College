...
void Assembler :: convertToCode() {
    ...
    while(!sourceFile.eof() && getline(sourceFile, currentLine, '\n')) {
        ...
        if(instruction == "START") {
            ...
            //ss << LC + 1; ss >> temp;
            IC.push_back("(" + returnType("START") + "," + returnOpcode("START") + ")\t(C," + string(itoa(LC, buffer, 10)) + ")");
            startPresent = true;
            instructionNumber++;
            continue;
        }
...
void Assembler :: generateMachineCode () {
    ...
    readFromIC.open("ic.txt", ios::in);

    // while(getline(readFromIC, ICCode, '\n')) {
    //     cout << ICCode << " hi";
    // }

    cout << readFromIC.rdbuf();
}
